...
def generate_fig1():
    ...
    plt.savefig("figure/fig1_lv_phase_enhanced.svg", bbox_inches='tight')
    plt.savefig("figure/fig1_lv_phase_enhanced.pdf", bbox_inches='tight')
    plt.close()
...
###############################################################
#       FIGURE 2 – Multi-species Competition + ESS
###############################################################
...
def generate_fig2():
    ...
    plt.tight_layout()
    plt.savefig("figure/fig2_multispecies_enhanced.png", dpi=300, bbox_inches='tight')
    plt.savefig("figure/fig2_multispecies_enhanced.svg", bbox_inches='tight')
    plt.savefig("figure/fig2_multispecies_enhanced.pdf", bbox_inches='tight')
    plt.close()
    
    print("Fig.2: Multi-species dynamics and diversity collapse")


###############################################################
#     FIGURE 3 – Coexistence Time vs K (with power law fit)
###############################################################
...
def generate_fig3():
    ...
    plt.tight_layout()
    plt.savefig("figure/fig3_survival_enhanced.png", dpi=300, bbox_inches='tight')
    plt.savefig("figure/fig3_survival_enhanced.svg", bbox_inches='tight')
    plt.savefig("figure/fig3_survival_enhanced.pdf", bbox_inches='tight')
    plt.close()
    
    print(f"Fig.3: K-dependent coexistence (β={b_est:.3f}, p<0.001)")


###############################################################
#   FIGURE 4 – Fixation Probability vs N (with Wright-Fisher theory)
###############################################################
...
def generate_fig4():
    ...
    plt.tight_layout()
    plt.savefig("figure/fig4_bottleneck_enhanced.png", dpi=300, bbox_inches='tight')
    plt.savefig("figure/fig4_bottleneck_enhanced.svg", bbox_inches='tight')
    plt.savefig("figure/fig4_bottleneck_enhanced.pdf", bbox_inches='tight')
    plt.close()
    
    print("Fig.4: Neutral drift dynamics")


###############################################################
#      FIGURE 5 – Coalescence Time (with linear regression)
###############################################################

def coalescence_time(N):
    """Time for 2 lineages to coalesce"""
    if N <= 1:
        return 0
...
def generate_fig5():
    """Coalescence analysis with theory comparison"""
    Ns = np.array([5, 10, 15, 20, 30, 40, 50])
    means = []
    ses = []
    
    for N in Ns:
        times = [coalescence_time(N) for _ in range(500)]
    ...
    # Linear regression: E[T] = 2N
    slope, intercept, r_value, p_value, std_err = stats.linregress(Ns, means)
    
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 4.5))
    
    # Panel A: Drift trajectories example
    ax1_main = ax1
    N_example = 10
    
    # Show multiple replicates
    for _ in range(10):
        traj = [0.5]
        for t in range(35):
            X = np.random.binomial(N_example, traj[-1])
            traj.append(X / N_example)
            if traj[-1] == 0 or traj[-1] == 1:
                break
        ax1_main.plot(traj, alpha=0.3, color='gray', lw=1)
    
    # Highlight one example
    example_traj = [0.5]
    for t in range(35):
        X = np.random.binomial(N_example, example_traj[-1])
        example_traj.append(X / N_example)
        if example_traj[-1] == 0 or example_traj[-1] == 1:
            break
    ax1_main.plot(example_traj, lw=2.5, color='steelblue', label='Example')
    ax1_main.axhline(0.5, color='red', ls='--', alpha=0.5, label='Initial freq.')
    
    ax1_main.set_xlabel("Generation")
    ax1_main.set_ylabel("Frequency of F₁")
    ax1_main.set_title(f"A. Drift trajectories (N={N_example})", 
                       loc='left', fontweight='bold')
    ax1_main.set_ylim(-0.05, 1.05)
    ax1_main.legend()
    
    # Panel B: Coalescence time vs N
    ax2.errorbar(Ns, means, yerr=ses, fmt='o-', color='navy', 
                ecolor='gray', capsize=3, markersize=7, label='Simulation')
    
    # Theory: E[T] = 2N
    theory = 2 * Ns
    ax2.plot(Ns, theory, 'r--', lw=2, label='Theory: E[T]=2N')
    ...
    plt.savefig("figure/fig5_coalescence_enhanced.svg", bbox_inches='tight')
    plt.savefig("figure/fig5_coalescence_enhanced.pdf", bbox_inches='tight')
    plt.close()
...
###############################################################
#       FIGURE 6 – Drift vs Selection Across N
###############################################################
...
def generate_fig6():
    ...
    plt.tight_layout()
    plt.savefig("figure/fig6_drift_selection_enhanced.png", dpi=300, bbox_inches='tight')
    plt.savefig("figure/fig6_drift_selection_enhanced.svg", bbox_inches='tight')
    plt.savefig("figure/fig6_drift_selection_enhanced.pdf", bbox_inches='tight')
    plt.close()
    
    print("Fig.6: Drift-selection balance (Ns determines regime)")


###############################################################
#  FIGURE 7 – Replacement Threshold (with Kimura theory)
###############################################################
...
def generate_fig7():
    ...
    for N_ex, col in zip(N_examples, colors_ex):
        ...
        # Mark threshold
        th = 1/(2*N_ex)
        ax3.axvline(th, color=col, ls=':', alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig("figure/fig7_threshold_enhanced.png", dpi=300, bbox_inches='tight')
    plt.savefig("figure/fig7_threshold_enhanced.svg", bbox_inches='tight')
    plt.savefig("figure/fig7_threshold_enhanced.pdf", bbox_inches='tight')
    plt.close()
    
    print("Fig.7: Selection threshold (Kimura approximation)")


###############################################################
#        FIGURE 8 – ESS Analysis with Replicator Dynamics
###############################################################
...
def generate_fig8():
    ...
    plt.tight_layout()
    plt.savefig("figure/fig8_ess_enhanced.png", dpi=300, bbox_inches='tight')
    plt.savefig("figure/fig8_ess_enhanced.svg", bbox_inches='tight')
    plt.savefig("figure/fig8_ess_enhanced.pdf", bbox_inches='tight')
    plt.close()
    
    print("Fig.8: ESS and stochastic outcomes")


###############################################################
#          FIGURE 9 – Fixation Time Distribution
###############################################################
...
def generate_fig9():
    ...
    plt.tight_layout()
    plt.savefig("figure/fig9_fixation_times_enhanced.png", dpi=300, bbox_inches='tight')
    plt.savefig("figure/fig9_fixation_times_enhanced.svg", bbox_inches='tight')
    plt.savefig("figure/fig9_fixation_times_enhanced.pdf", bbox_inches='tight')
    plt.close()
    
    print("Fig.9: Fixation time statistics")
